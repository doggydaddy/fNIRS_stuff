...
for i in $files; do
    ...
    3dDeconvolve -input ${x_pref}.nii -nfirst 0 -polort 3 \
                 -num_stimts 4 \
                 -stim_file 1 ${x_pref}.glm2 -stim_label 1 congru \
                 -stim_file 2 ${x_pref}.glm3 -stim_label 2 incong \
                 -stim_file 3 ${x_pref}.ecg -stim_base 3 -stim_label 3 hrv \
                 -stim_file 4 ${x_pref}.eda -stim_base 4 -stim_label 4 eda \
                 -num_glt 3 \
                 -gltsym 'SYM: +congru' -glt_label 1 stroop-rest \
                 -gltsym 'SYM: +incong' -glt_label 2 stroop-rest \
                 -gltsym 'SYM: +incong -congru' -glt_label 3 incong-congru \
                 -tout -bout \
                 -bucket stat_${x_pref}_stroop.nii

    rm $x_base $x_pref.glm1 $x_pref.eda $x_pref.ecg
done
rm *.REML_cmd *.xmat.1D
